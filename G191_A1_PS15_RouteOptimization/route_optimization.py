...
def parse_input_file(path: str | Path) -> ProblemSpec:
    ...
    Non-numeric header lines (e.g. 'City Coordinates:') are skipped.
    Also supports coordinate-only rows (without index):
      5 10
    """
    file_path = Path(path)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--restarts", type=int, default=5, help="Number of hill-climbing restarts.")
    parser.add_argument(
        "--no-random-fallback",
